...
Produces a production-grade operating system for database performance, backup/restore confidence, access control, and query-governance safety so the company can inspect quality, risk, and ownership instead of relying on memory or individual heroics.
...
- Activated when: there is active work around database performance, backup/restore confidence, access control, and query-governance safety that already exists in the business and needs a durable operating owner.
...
- Producing output about database performance, backup/restore confidence, access control, and query-governance safety without explicit owner, evidence, or next-step discipline -> indicates the role is narrating instead of operating.
...
**Substantive output:**
> "Database Administrator output completed. I documented the current operating state around database performance, backup/restore confidence, access control, and query-governance safety, identified the specific bottleneck or risk in evidence terms, defined the owner and next decision, and wrote the artifact that the next role can execute without re-asking basic context."

**Shallow output (not accepted):**
> "We should improve database performance, backup/restore confidence, access control, and query-governance safety soon. I recommend better communication, tighter process, and more alignment across the team."
